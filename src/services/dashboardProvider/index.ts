...
export function DashboardProvider() {
  ...
  const { setMyGroupTasksData, setAllRequestsData, setTasksWithRequestData } = useDashboardStore(
    (state) => state
  );
  ...
  const { mutateAsync: getAllRequestTab } = useMutation(
    ...
    async () =>
      <AxiosResponse<any>>
      await api.get(`case-management/v1/searchcase?UserOwner=${userData.userName?.toLowerCase()}&Status=1&TypeRequest=2`),
    {
      onSuccess: (response) => {
